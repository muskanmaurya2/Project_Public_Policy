...
try:
    ...
    # Construct a more robust, relative path to the model file
    # This assumes 'Quantum_AI_Policy' is a sibling directory to 'AI_infosys_Policy'
    # CORRECTED: 'Quantum_AI_Policy' is a subdirectory, so we remove the ".."
    ...
    quantum_model_path = os.path.normpath(quantum_model_path) # Clean up path (e.g., ..\)
...
def quantum_page(request: Request, query: str = None):
    ...
    # Validate that essential quantum model components are available
    if quantum_df is None or quantum_vectorizer is None:
        return templates.TemplateResponse(
            "quantum.html",
            {"request": request, "results": None, "error": "Quantum model components (dataframe or vectorizer) are missing or malformed. Please check 'quantum_nlp_models.pkl'."}
        )
